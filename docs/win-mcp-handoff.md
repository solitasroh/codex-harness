...
★ 이 6개를 **`bin/doctor.ps1` 이 자동 판정**하도록 만들었다(baek 2026-07-07). 실사용자는 아래를
수동으로 하지 말고 doctor 1회 실행 → PASS/FAIL/SKIP 요약을 보면 된다. (수동 체크는 doctor 가
없거나 개별 디버깅할 때만.)
```
# 권장: 설치 직후 자가진단 1회
claude plugin marketplace update       # (git pull 후)
.\bin\codex_bootstrap.ps1              # 기존 codex 로그인 재사용 + 훅/패턴 설치
.\bin\doctor.ps1                       # 6항목 실동작 자동 판정 (FAIL 있으면 exit 1)
```
doctor 가 판정하는 6항목(각 PASS/FAIL/SKIP + 근거):
```
1. codex CLI PATH 존재
2. CODEX_HOME\auth.json 존재·읽기 가능
3. 실제 MCP tools/call 성공 — ★ threadId 만 아니라 isError=false·401없음까지 파싱(phantom 방지)
4. dotnet + qa_verify.py 가 임시 xUnit 을 실제로 빌드·테스트해 exit 0
5. 가드훅 패턴 자가탐색 — 안전명령 통과 / 위험명령 차단 (env 없이)
6. CODEX_HOME 갈림길 리포트 (읽기 전용, 사람 판단)
```
리눅스에서 짠 코드만으론 "완료" 아님. doctor 가 윈도우 실기에서 **FAIL 0** 나야 스코프 C 종료.
(doctor 자체의 판정 로직 6종은 리눅스 pwsh+dotnet 로컬설치로 양방향 실증 완료:
 3=진짜PASS/가짜FAIL, 4=xUnit PASS, 5=패턴있음PASS/없음FAIL. 윈도우 실기에서만 못 본 건
 icacls·py런처·%USERPROFILE% 기본값뿐 — doctor 가 그것도 실사용자 PC에서 대신 확인함.)
